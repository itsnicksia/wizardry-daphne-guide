...
??? info "TheAxolotl's Analysis"
    ...
    === "Fangs of the Clear Mirror"

        I'll keep this nice and short. Shiou is one of my favorite adventurers, and that doesn't change with her new style.

        Veiled Moon is a great addition to Shiou's kit. It effectively gives her a version of Drastic Strike that can be used as a Samurai, and it synergizes quite well with her standard kit of Lingering Blossom and Ephemeral Illusion Stance. At full health, Veiled Moon doesn't do anything noteworthy, but at lower health thresholds, it can surpass Bamboo Strike's damage. This can let you do some fun things with enemy targeting, such as Lingering to draw hits to Shiou, then Veiled make Shiou less likely to be targeted, then Ephemeral and/or more Veiled attacks. As with other "taunt" mechanics like this, using Veiled will not guarantee Shiou does not get attacked, but it does increase the chance someone else will. If you happen to pair Shiou with Galbadius, Veiled plus Galb's kit will work in tandem to make Galb more likely to be attacked and Shiou less likely to be attacked.

        Full Mastery: Ephemeral Illusion Style is effectively identical to Ephemeral Illusion Stance in terms of damage output, but there are a couple of notable differences. First, the trigger methods are different. While the original version was based on a health threshold, this version is based on the number of actions taken. There are positives and negatives to each approach. The new style is likely going to be the safer way to trigger the Ephemeral hit, however it can take a little bit longer to ramp up. The standard form is going to be riskier due to the lower health requirement, however it'll be easier to trigger sooner. Inheriting something like Flutterdream Flash does count towards the "action count" required to trigger Full Mastery Ephemeral, however passives like Follow-Up Attack and Counterattack do not. I believe there are a few exceptions, but if an action would cause a decrease in a buff or debuff counter, that would also contribute towards the Full Mastery Ephemeral.

        The other major difference is that the standard Ephemeral is a stance, while the Full Mastery Ephemeral is not. This means that Full Mastery Ephemeral can be used while in Cresting Wave Stance without canceling that stance. The standard Ephemeral being a stance itself means it will cancel Cresting Wave. Depending on the encounter, that may or may not be beneficial.

        One key thing to note is that both the standard Ephemeral Illusion Stance and the new Full Mastery: Ephemeral Illusion Style can be leveled with codexes, however they do not share a skill level. If you feed codexes to one version, then switch styles, the skill level will not pass over to the other style.

        The new discipline is a solid Surety and ASPD combination, as well.

        I'm personally a big fan here, but Shiou, like other Samurai, are not as easy to work with as Fighters are. They have higher damage potential, but higher risk, so choose wisely and savor the risk!
